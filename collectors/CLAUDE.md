...
- **W08 종목 어댑터 3종**(MCP 검증): `inquire_price`(FHKST01010100, `/quotations/inquire-price`, 단일 output — 현재가·`per`·`w52_hgpr`/`w52_lwpr`·`hts_avls`), `finance_income_statement`(FHKST66430200, `/finance/income-statement`), `finance_financial_ratio`(FHKST66430300, `/finance/financial-ratio`). 재무 2종 output은 **리스트(행 1개면 단일 dict로 오는 변형** → `normalize._output_rows`가 리스트화). ROE는 `roe_val`.
  - **`normalize_financial_ratio` additive 확장(재무 100점용)**: 같은 FHKST66430300 응답이 수익성 외에 **성장성·안정성**도 담아준다 → 정규화기가 `revenue_growth`(grs)·`op_income_growth`(bsop_prfi_inrt)·`net_income_growth`(ntin_inrt)·`debt_ratio`(lblt_rate)·`reserve_ratio`(rsrv_rate)를 추가로 뽑는다(**추가 API 콜 0**). raw KIS 명 노출 금지 — clean snake 로. 소비: `stock.financial_score`(수익성 ROE·성장성 net_income_growth·안정성 debt_ratio). 기존 eps/bps/roe 소비자(build_stock_summary·avg_per) 불변(additive).
  ...
  - **[2026-07-07 라이브 검증 통과]** `tests/live/test_live_stock_bundle.py` 4게이트(real 키, 삼성전자 005930): 재무 히스토리 **23년**, EPS/주가 조정기준 일치(PER_year 자릿수 튐 없음), 재무 API **real 정상**, 일봉 **100/회**(6개월 창은 ~100 상한 → 더 길면 페이지네이션).
  - **`fetch_chart_series`(페이지네이션)** — KIS ~100/콜·**연속 토큰 없음**이라, 장기간(일봉 1~10년·주봉 등)은 **date-window 후진 페이지네이션**으로 넘는다: `end`(FID_INPUT_DATE_2)를 '가장 오래된 수신일 직전'으로 당겨 `inquire_daily_itemchartprice` 를 반복→병합·중복제거·date 오름차순. 종료: `oldest<=start_date`(구간 소진)·**무진행**(`len==before`, 무한루프 방지)·`max_bars`/`max_pages` 캡. `period` D/W 공용. **라이브 실증(005930)**: D/1y 243봉(3콜)·D/3y 728(8콜)·W/10y 521(6콜)·**D/10y 2452(~25콜·~2.5s)**. 소비: `GET /api/detail/{ticker}/chart`(선택형 차트). 일봉 무캐시(원칙1) 유지. 결과: `constants.AVG_PER_VERIFIED=True`(엔진이 avg_per/valuation_label 산출). 어댑터 조정기준/소스 변경 시 재검증.
