...
**UI 디자인 톤:** 모든 화면은 **흰색/회색/파랑/남색/검정 5계열 팔레트**로 통일한다(빨강·초록·황색 배제, 상승/하락도 파랑·회색으로). 색은 `frontend/src/theme.css` 토큰(`var(--c-...)`)이 단일 출처 — 하드코딩 금지. 상세는 `ui-design-system` 스킬. UI 작업 시 frontend-engineer가 이 스킬을 먼저 읽는다.
