...
|------|-----------|------|------|------------|
| backend-architect | general-purpose | **백엔드 총괄 리드·아키텍처 거버넌스·리팩토링·기술부채**(data/quant/llm 조율, QA와 협업) | backend-architecture + refactoring | 교차 관심사·리팩토링·아키텍처 점검 시 |
...
**TDD 원칙 (전 구현 작업 공통)**: 모든 구현 팀원은 `tdd-workflow` 스킬의 Red→Green→Refactor 사이클을 따른다. 구현 코드보다 실패하는 테스트가 먼저다. **Refactor 단계·구조 정리 작업은 `refactoring` 스킬**(행동 보존·characterization 테스트·작은 단계·기능 분리)을 따른다 — backend-architect가 지시하든 구현자(백엔드/프론트) 본인이 하든 공유한다.

**backend-architect vs qa-inspector (역할 분리)**: QA는 "동작이 맞는가"(경계면 shape·안전 grep·테스트 실행, 반응형 검증), 아키텍트는 "구조가 건강하고 올바르게 갈라졌는가"(계층·SSOT·계약 설계·리팩토링·기술부채, 선제적 거버넌스). 둘은 겹치지 않고 맞물린다 — QA 발견 → 아키텍트가 근인(구조적 결함) 판단 → 리팩토링 티켓 → 담당 수정 → QA 회귀. 같은 QA 실패가 2회↑ 반복되면 아키텍트가 개입해 재발을 구조로 막는다.
...
- **backend-architect 투입 기준(상시 아님)**: ① 변경이 **여러 백엔드 모듈에 걸치는**(교차 관심사) 주차/작업, ② **리팩토링·아키텍처·기술부채 점검** 요청, ③ **QA가 반복되는 구조적 이슈**를 발견했을 때 스폰한다. 단일 모듈의 좁은 구현·버그픽스에는 굳이 넣지 않는다(조율 오버헤드). 투입 시 백엔드 구현 에이전트(data/quant/llm)의 인터페이스를 **사전 설계**하고 QA와 상시 협업한다.
...
[리더] Agent 스폰(named, background) + TaskCreate
        │
   ┌────────────────── backend-architect (백엔드 총괄·계약 사전설계·계층/SSOT 거버넌스) ──────────────────┐
   │                                                                                                   │
...
각 모듈 완성 즉시 ──────────────▶ qa-inspector ──파일:라인 수정 요청──▶ 담당자                             │
                                     │   ▲                                                              │
                                     │   └── 리팩토링 티켓·회귀 재검증 ◀──▶ backend-architect ────────────┘
                                     │        (QA 발견 → 근인 판단 → 티켓 → 담당 수정 → QA 회귀)
                            _workspace/{week}_*_report.md · *_backend-architect_review.md
                                     │
                              [리더: 통합 보고]
```
...
| 팀원 간 계약 충돌 (shape 불일치) | 생산자 명세(`_workspace/`)가 기준. 소비자가 맞추되, 명세가 플랜과 다르면 플랜 우선. 계약 자체가 부실하면 backend-architect가 계약을 재설계 |
| 같은 유형 QA 실패 2회↑ 반복 | backend-architect 투입 → 구조적 근인 진단 + 재발 방지 리팩토링(개별 수정 반복 금지) |
| 리팩토링 후 테스트 red (행동 미보존) | backend-architect가 롤백 지시 + 더 작은 단계로 재분할. 리팩토링과 기능 변경은 분리 |
| 안전 검증 실패 (주문 API 등) | **모든 작업 중단** 후 즉시 수정 — 다른 작업보다 우선. backend-architect가 구조적 재발 방지 설계 |
...
### 아키텍처/리팩토링 흐름 (backend-architect)
1. 사용자: "백엔드 챗 계층 리팩토링 점검해줘" (또는 QA가 경계면 버그를 2회 연속 보고)
2. Phase 0: `_workspace/` 확인 → 리팩토링/점검 요청이므로 backend-architect + 관련 구현 에이전트(llm) + qa-inspector 스폰
3. backend-architect가 `backend-architecture` 스킬을 읽고 `chat/` 계층 진단 → `chat()`·`chat_stream()`의 툴 되먹임 로직 중복(SSOT 위반)을 근인으로 특정 → characterization 테스트 선행 조건으로 공통 헬퍼 추출 리팩토링 티켓 발행 → llm-engineer에 배정
4. llm-engineer가 행동 보존(테스트 green 유지)으로 리팩토링 → qa-inspector가 회귀 검증(계약 불변 확인)
5. backend-architect가 `_workspace/*_backend-architect_review.md`에 기술부채 카탈로그(TD-xx) 기록 → 리더 보고
