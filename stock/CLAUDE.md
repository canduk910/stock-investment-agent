...
- **보통주 유니버스 필터 (`universe.py`, 순수)**: `common_stocks(master)` = `stock_master.load_stock_master()`(KOSPI+KOSDAQ ~3,900, KIS 콜 0·`{ticker,name,market}` 3필드) 에서 **보통주만**. 마스터에 증권종류 필드가 없어 **코드 규칙 + 이름 휴리스틱**(판정=순수 문자열/코드·LLM 0): ①**우선주 제외** — KRX 보통주 코드는 관용적 **끝자리 0**(우선주/신형우선주는 5/7/9/K 등), `ticker[-1] != "0"` → 제외. ②**ETF/ETN/스팩/리츠 제외** — 이름이 `ETF_BRANDS`(startswith·KODEX/TIGER/KIWOOM/… ETF 전용 브랜드) 로 시작하거나, `ETF_BRANDS_SPACED`(**공백 뒤에만** — BNK/TIME/WON: 정식 종목 `BNK금융지주` 보존하며 `BNK 스마트카`[ETF] 만 제외) 로 시작하거나, `NAME_EXCLUDE_KEYWORDS`(스팩·ETN·선물·레버리지·인버스·채권·리츠·**액티브**·**(합성)**·**(H)**) 를 포함하면 제외. **★함정**: `합성`(동남합성=화학사)은 단독 금지어 금지 → `(합성)`(괄호 포함) 만. 짧은 브랜드(BNK)는 startswith 로 정식 종목을 삼키므로 반드시 공백 매칭. 상수 3종 SSOT(조정 가능)·**과필터보다 명확한 제외 위주(보수적)**. 라이브 실측: master 3,922 → common **2,578**(KOSPI 833·KOSDAQ 1,745 — 실제 보통주 규모와 정합·누수 0).
