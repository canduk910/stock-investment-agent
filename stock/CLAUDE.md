...
## 재무 100점 점수 (financial_score.py) — 스크리너 후보 정량화
- **`financial_score(ratio, per_vs_avg=None) → {score, axes[], available_axes, note}`**(순수·LLM/랜덤 0, macro `score_axes` 미러). 4축 균등(각 25점): 수익성(ROE)·성장성(순이익증가율)·안정성(부채비율)·밸류에이션(PER vs 자기평균). **임계·배점은 `constants.FINANCIAL_SCORE_SPEC` 단일 출처** — 함수는 SPEC 티어를 읽어 점수화(숫자 하드코딩 금지). `_axis_points(key,value)`: direction "high"=`value>=threshold`, "low"=`value<threshold` 첫 티어, 미해당은 floor, None 은 무투표(None).
- **재정규화(임의값 금지)**: 가용 축(값 있는 축) 합/최대×100. **밸류 축은 per_vs_avg 주입 시만**(스캔 저장분엔 없음 → 3축, 서빙이 라이브 PER 로 4축). avg_per None·전 축 결측 graceful(전 축 결측 → score None+사유). 최신 연간 행은 `_latest_annual_row`(=`_recent_annual_periods` 재사용, 분기 interim 제외).
- **SSOT 3중 일관성**: `FINANCIAL_SCORE_SPEC`(임계/배점) → `FINANCIAL_SCORE_CONFIG`(SPEC 파생·프론트 축 라벨 노출·`api/screener` 응답 `score_config`) → 잠금 테스트(`test_spec_axis_max_matches_top_tier`=AXIS_MAX==각 축 최고 티어·`test_config_derived_from_spec`). **판정=코드(점수)이지 매수/매도 판정 아님** — 프론트가 정량 지표 요약으로 표시·면책. **주의(범용 한계)**: 은행/금융주는 부채비율이 구조적으로 높아 안정성 축 저점(축 breakdown 으로 확인).
...
- **`grand_cycle_for_chart(chart)`**(public·`stage_segments_for_chart` 의 richer 버전): 차트 candles → **`_ma_grand_cycle` 전체 dict**(stage+밴드폭/방향/지속봉수) 또는 None(봉<40). `_sorted_dated_closes`로 (date,close) 뽑아 `_ma_grand_cycle` 를 public 노출. **대순환 후보 스크리너(`stock/screener.py`)**가 종목별 일봉으로 현재 단계+밴드를 얻는 진입점(private `_ma_grand_cycle` 직접 import 회피).

## 대순환 후보 스크리너 (screener.py) — 여러 종목 단계 스캔 (live top-N)
- **`screen_grand_cycle(client, *, market_iscd="0000", size=30)`** → `{candidates:[{ticker,name,price,change_rate,market_cap, stage,stage_name,arrangement,band_width_pct,band_direction,bars_in_stage}], catalog:INDICATOR_CONFIG['grand_cycle'], market_iscd, size, partial_failure, as_of}`. **판정=코드·LLM 0**. 파이프라인: `ranking.market_cap_rank` **1콜**로 유니버스(종목명 포함) → 종목별 `chart.fetch_chart_series`(90일창≈60봉·**단일 콜**) 병렬(`infra.parallel.fetch_parallel`·캡 `WATCHLIST_FETCH_CONCURRENCY`=6·KIS 유량 보호) → `grand_cycle_for_chart` 로 현재 단계 산출. 봉<40·조회실패 종목은 `stage=None`(판정보류)·실패는 `partial_failure`(per-item graceful). **단계 필터는 응답에 미적용** — 전 유니버스를 단계와 함께 반환(프론트가 클라이언트에서 필터). **일봉 무캐시**(원칙1·매 요청 재계산). 매매 API 0(랭킹·일봉 조회만). 상수(5/20/40·40봉 최소·6단계 라벨)는 `constants` import 소비(3중 일관성). **시총상위 ~30종목 상한**(랭킹 페이지네이션 없음) — 전체 시장은 아래 배치 스캐너로.

## 전체 유니버스 스크리너 — 보통주 필터 + 배치 스캔 + DB (universe·screener_scan·screener_store·screener_models)
- **보통주 유니버스 필터 (`universe.py`, 순수)**: `common_stocks(master)` = `stock_master.load_stock_master()`(KOSPI+KOSDAQ ~3,900, KIS 콜 0·`{ticker,name,market,sec_group}`) 에서 **보통주만**(판정=순수 문자열/코드·LLM 0). 우선순위:
  - ①**우선주 제외 (코드 규칙, 전 경로 공통)** — KRX 보통주 코드는 관용적 **끝자리 0**(우선주/신형우선주는 5/7/9/K 등), `ticker[-1] != "0"` → 제외. 우선주도 법적으로 그룹은 ST(주권)라 그룹코드로는 안 걸리므로 이 코드 규칙이 여전히 필요.
  - ②**증권그룹구분코드 결정적 필터 (sec_group 있으면 우선)** — `sec_group == "ST"`(주권)만 통과, ETF(EF)·ETN(EN)·리츠(RT)·ELW(EW)·수익증권(BC)·외국주권(FS)·예탁증서(DR)·투자회사(MF)·인프라(IF) 등 전부 제외. **스팩(SPAC)은 법적 주권=ST → 그룹코드로 안 잡히므로 이름 `SPAC_NAME_KEYWORDS`("스팩") backstop 유지**. 이름 브랜드 휴리스틱의 **누수**(신생 브랜드 ETF `1Q 미국배당TOP30` 등)와 **오탐**('메리츠'/'블리츠'의 '리츠' 부분일치)을 결정적으로 해소.
  - ③**폴백 (sec_group 부재 → 이름/코드 휴리스틱)** — 구 캐시·파싱실패로 None 이면 `_passes_name_heuristic`(기존 로직): `ETF_BRANDS`(startswith·KODEX/TIGER/KIWOOM/…) / `ETF_BRANDS_SPACED`(**공백 뒤에만** — BNK/TIME/WON: `BNK금융지주` 보존하며 `BNK 스마트카`[ETF] 만 제외) / `NAME_EXCLUDE_KEYWORDS`(스팩·ETN·선물·레버리지·인버스·채권·리츠·액티브·(합성)·(H)). **★함정**: `합성`(동남합성=화학사)은 단독 금지어 금지 → `(합성)`(괄호) 만. 짧은 브랜드(BNK)는 반드시 공백 매칭.
  - 상수 SSOT(`SECURITY_GROUP_COMMON="ST"`·`SPAC_NAME_KEYWORDS`·ETF 3종)·**과필터보다 명확한 제외 위주(보수적)**. 라이브 실측: master **3,927** → common **2,532**(KOSPI 805·KOSDAQ 1,727 — sec_group 결정적: ETF/DR/FS/IF 등 48건 누수 제거 + '리츠' 오탐 2건 복구). 라이브 게이트: `test_live_common_stocks_no_etf_leak_and_scale`.
- **배치 스캐너 (`screener_scan.py`)**: `scan_all(client, store, *, universe=None, on_progress=None, concurrency=8, as_of_date=None) → counts{total,scanned,with_stage,failed,as_of_date,stored}`. universe 미지정이면 `common_stocks(load_stock_master())`. 종목별 **`screener.fetch_scan_metrics`(★종목당 4콜)** 을 `ThreadPoolExecutor`+`as_completed`(캡 `DEFAULT_CONCURRENCY`=8·KIS 유량 보호·backoff 는 client) 로 병렬 → `store.upsert_scan(as_of_date, rows)`. per-item graceful(fetch 예외=`failed`++·stage None·전체 계속), 봉<40 은 stage None(failed 아님). as_of_date 기본 = **KST 오늘**("YYYY-MM-DD"). **엔트리포인트 `python -m stock.screener_scan`**(`_main`): `init_db()` → `resolve_kis_client(None,db)`(env/`__shared__`) → `ScreenerResultStore()` → 진행률 로깅. 로컬·Cloud Run Job 공용(300s 제약 없음). 매매 API 0·현재가/일봉 무캐시(확정/파생 스냅샷만 DB).
- **`fetch_scan_metrics(client, ticker)`(screener.py·후보 강화 4콜)**: ①일봉(`fetch_chart_series` — 대순환 stage + **spark**[최근 30 종가·일봉 부산물]) ②`inquire_price`(**market_cap** 정렬키) ③`finance_financial_ratio`(최신 연간 **roe·net_income_growth·debt_ratio** — 재무 3축 원천) ④월봉 결산가(`_year_end_prices`·원주가) + `_avg_per`(**avg_per** 밸류 축용). **per-call graceful**: 일봉(1차·stage) 실패만 예외 전파(그 종목 failed), 시총·재무·avg_per 는 개별 try(실패해도 나머지 보존). **현재가·현재 PER 은 담지 않는다**(무캐시 원칙1 — 서빙이 라이브). 라이브 실증: 0.13s/종목 → 2,532종목 ≈ **5.7분**(concurrency 5)·avg_per 가용률 ~90%(대·중형). live `screen_grand_cycle`(top-N)은 이 함수 미사용(경량 유지).
- **저장 (`screener_store.py`·`screener_models.py`)**: `ScreenerResultRow`(테이블 `screener_results`·유니크 `(as_of_date, ticker)`·`import_models` 등록·create_all) — as_of_date/ticker/name/market/stage/stage_name/arrangement/band_width_pct/band_direction/bars_in_stage/scanned_at **+ 후보 강화 스냅샷 컬럼 `market_cap`(정렬키·억원)·`roe`·`net_income_growth`·`debt_ratio`(재무 3축 원천)·`avg_per`(밸류 축용)·`spark`(미니차트 종가·콤마 조인 String)**. 신규 컬럼은 `infra/db._ADDITIVE_COLUMNS`(idempotent ADD COLUMN·SQLite/Postgres 공용)로 기존 DB 백필. **★현재가·현재 PER 은 저장 안 함**(무캐시 원칙1 — 서빙이 표시 top-N 만 라이브 조회). `_entry_from_row`(계약 SSOT)가 이 컬럼들을 반환(+`_spark_from_str`/`_spark_to_str` 왕복). `ScreenerResultStore`(`ScopedReportRepo` 패턴·**메서드마다 새 세션**·스레드 안전): `upsert_scan`(그 날짜 기존 행 전량 교체=같은날 재스캔 idempotent), `latest_as_of()`, `list_results(as_of_date=None, market=None, stage=None)`(최근 스캔 기본·필터 옵션·stage None 행도 반환[후보 제외는 서빙]). `default_store()` 싱글턴.
