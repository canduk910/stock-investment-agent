...
- **연세대 소속 마크(톱바 좌측 클러스터)**: DK 브랜드 옆에 세로 구분선(`.app__affil-divider`) + **연세 엠블럼**(`YonseiEmblem` 자체 제작 인라인 SVG — 연세 블루 원형 배지+"연세", **공식 CI 복제 아님**·과제 소속 표기용 원본 마크, 공식 CI 파일 생기면 이 컴포넌트만 교체) + "연세대학교 AI핀테크"/"[AI핀테크Agent분석과 설계]" 2줄. 색은 신규 토큰 **`--c-yonsei`**(theme.css SSOT·파랑/남색 계열, **브랜딩 전용 — 가격 방향색 아님**). DK 브랜드+소속을 `.app__brand-group` 한 덩어리로 좌측 고정, 상태 칩은 우측 유지.
