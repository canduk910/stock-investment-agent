...
- **연세대 소속 마크(톱바 좌측 클러스터)**: DK 브랜드 옆에 세로 구분선(`.app__affil-divider`) + **연세 엠블럼**(`YonseiEmblem` = **연세대학교 공식 CI 이미지** `src/assets/yonsei-ci.png`[사용자 제공], `<img class="app__yonsei" alt="연세대학교 로고">` — 높이 34px 고정·비율 유지. 자체 제작 SVG 배지에서 교체) + "연세대학교 AI핀테크"/"[AI핀테크Agent분석과 설계]" 2줄. 대학명 텍스트 색은 **`--c-yonsei`**(theme.css SSOT·파랑/남색 계열, 브랜딩 전용·가격 방향색 아님 — 엠블럼은 이미지라 토큰 무관). DK 브랜드+소속을 `.app__brand-group` 한 덩어리로 좌측 고정, 상태 칩은 우측 유지.
