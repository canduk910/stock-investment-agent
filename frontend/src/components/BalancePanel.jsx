...
// 요약 카드(예수금·매입액·평가액·손익·순자산) + 보유종목 표(종목·수량·평단·현재가·평가액·손익/수익률).
// 손익 색 = 글로벌 팔레트(상승=파랑 --c-up / 하락=회색 --c-down) — WatchlistView 등락률과 동일 규칙.
//   차트 예외(빨강 상승) 아님. 손실은 회색(중립)으로 표기하고 빨강(위험)은 쓰지 않는다.
// 부분실패(KIS 조회 실패, partial_failure:['balance'])는 "일시 조회 불가" 안내(전체 에러 화면 아님).
...
export default function BalancePanel() {
  ...
  if (balanceFailed) {
    return (
      <div className="balance">
        <div className="banner banner--warn balance__partial" role="status">
    ...
    <div className="balance">
      ...
      {summary ? (
        <div className="balance__summary">
          ...
          <div className={`balance__card balance__card--pnl ${dir(summary.pnl_amount)}`}>
            <span className="balance__card-label">평가손익</span>
            <span className="balance__card-value balance__pnl-inline">
              <Pnl amount={summary.pnl_amount} />
            </span>
          </div>
          <SummaryCard label="순자산" value={won(summary.net_asset)} tone="emph" />
        </div>
      ) : null}
