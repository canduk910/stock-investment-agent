...
- **표본 간격(다운샘플) — `trajectory_step(months)`·`downsample_trajectory(points, step)`**: 월별 원본은 창(개월)이 길수록 과밀·불규칙(라벨이 국면 전환마다)이라 **범위별로 성기게 표본화**한다 — `trajectory_step`: 1년(≤12개월)=분기(step 3, `"quarterly"`)·2년(≤24)=반기(6, `"semiannual"`)·3년+ = 연(12, `"annual"`). `downsample_trajectory`: **가장 최근 점을 앵커**로 step 만큼 뒤로 등간격 표본(최근 확정월 보존 → 라이브 마커/브릿지 연결 유지), **정상 12/24/36개월 = 정확히 4/4/3점**. 가드: step≤1·**2점 이하는 무표본**(월별 유지), 창<step 과박이면 **가장 과거 점 보강**(최소 2점, 선이 그려지게). **판정은 안 바꾼다** — build_trajectory(엔진 재현) 결과의 표시 밀도만 조정. 라우트가 `build_trajectory` 후 적용해 응답에 `interval`+`step_months` 를 싣는다.
