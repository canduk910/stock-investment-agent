...
노트북에는 실행 출력이 이미 채워져 있어(실키 재실행) 키 없이도 결과를 확인할 수 있다.

## 보너스 — 결정적 엔진의 재사용

5요소 데모 뒤에 **국면 이동 궤적**(`macro/regime_history.build_trajectory`) 셀을 두었다. 라이브 국면 판정과
**동일한 결정적 엔진 `judge_regime`** 으로 과거 월별 지표를 재현해 궤적을 만든다(LLM·네트워크 0) — "판정은
코드"라는 안전 철학을 그대로 보여주는 예다.

## 현행화 내역

- 인텐트 분류기에 **7번째 라벨 `analyst_report` 신설**(데이터셋 보강·재학습) — 리포트 수집/검색 요청을 독립
  의도로 인식(1 섹션).
- 로드맵 이후 확장(시황 요약·챗 상담·'금일의 요약', 국면 궤적, 이동평균선 대순환, 선택형 차트, 회원제/RBAC,
  헤드라인 통계)을 인트로·결론에 반영하고 궤적 보너스 셀을 추가했다.
